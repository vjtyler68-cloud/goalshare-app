...
## Backend email (OTP) is unreliable — login never requires verification
The backend issues a full token to unverified accounts, so email verification is effectively optional; the app auto-logins right after registration rather than sending users to an OTP screen that may never complete. **Why:** backend email sending fails, and only compiled JS is deployed so it can't be fixed from this repo. **How to apply:** don't build flows that depend on backend email (OTP, password-reset); if backend email is restored, revisit the auto-login shortcut.

There is also a client-side test-account whitelist (`isTestAccount()` in `lib/core/utils/test_accounts.dart`) that skips the subscription gate for designated accounts — a deliberate user decision, but a known access-control risk if the pattern is guessable; flag it before any public release.
...
## App-wide: DateTime.parse + unsafe casts are the systemic "data disappears" bug
This backend returns timestamps/lists that can be null or non-ISO. `DateTime.parse` THROWS on non-ISO and `(x as List)` THROWS on null — inside a `fromJson`/`assignAll` one bad record blanks the ENTIRE list (looks like save/load failure). Rule: models use `DateTime.tryParse(json[...].toString())` (fields are nullable), `formatDate` helpers tryParse + return raw string on fail, list casts use `(response['data'] as List? ?? [])`. WATCH: making a date field reliably-nullable can expose a force-unwrap (`e.dueDate!`) in the UI — grep the widget layer for `!` on any field you convert. Clean: no hardcoded secrets, no http://, no print() in lib/.

## Multipart uploads: Accept header + defensive JSON parse
Every multipart upload to the Railway backend must send `Accept: application/json` AND parse the response body defensively (wrap json.decode in its own try, treat non-JSON as null, then check statusCode 2xx + `newRes is Map` + success==true). Railway edge / cold-start can return an HTML error page, so an unconditional json.decode throws into the generic catch and shows a misleading "Something went wrong", hiding the real HTTP status/message. The working motivation upload already sent Accept; the vision board upload didn't — that omission was the bug. Always surface the real backend `message`/status in the failure branch and log status+body.
